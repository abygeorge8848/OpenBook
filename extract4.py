        ...
        for block in blocks:
            if block['type'] == 0:
                ...
                for index, line in enumerate(lines):
                    ...
                    # Combine fragments if they form a continuous text with the current line
                    if pending_fragments and is_continuous(merge_fragments(pending_fragments), text):
                        text = merge_fragments(pending_fragments) + " " + text
                        pending_fragments.clear()
                    else:
                        pending_fragments.clear()

                    # Check if current line makes sense with the next one, but don't merge if it forms unintended words
                    if index < len(lines) - 1:
                        next_span = lines[index + 1]["spans"][0]
                        next_text = next_span["text"].strip()
                        next_fontsize = next_span["size"]

                        if is_continuous(text, next_text) and fontsize == next_fontsize:
                            text += " " + next_text
...
with open('raw_data/extracted_content4.json', 'w', encoding='utf-8') as f:
